...
int motor_0_dir = 1;        // left wheel, 1 - forward, 0 - backward
int motor_1_dir = 1;        // right wheel, 1 - forward, 0 - backward
...
int stop_flag = 0;          // 1 - stop, 0 - not stop
...
int desiredSpeedLWheel(char action) {
  ...
  switch (action) {
    ...
    case 'L':  // slower
      rate /= 100;
      if (des_speed * (1.0 + 0.5 * rate) > 16383.0) {
    ...
    case 'R':  // faster
      rate /= 100;
      Lspeed = des_speed * (1.0 + 0.5 * rate);
      if (Lspeed > 16383.0) Lspeed = 16383.0;
      return (int)Lspeed;
    default:
      return 0;
  }
...
int desiredSpeedRWheel(char action) {
  ...
  switch (action) {
    ...
    case 'L':  // faster, but cannot be faster than 16384
      ...
      if (Rspeed > 16383.0) Rspeed = 16383.0;
      return (int)Rspeed;
